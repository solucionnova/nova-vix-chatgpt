...
export class GatewayBridge {
  ...
  async #handle(req, res) {
    try {
      ...
      const requestId = randomUUID(); const timer = setTimeout(() => { const p = this.pending.get(requestId); if (!p) return; this.pending.delete(requestId); this.#markConsumed(requestId, 'expired'); if (!res.headersSent) json(res, 504, { error: { message: 'inference_timeout' } }); else res.end(); }, this.requestTimeoutMs); timer.unref?.();
      const item = { request_id: requestId, protocol, wire_format: protocol, request, headers: safeHeaders(req.headers) }; this.pending.set(requestId, { requestId, protocol, request, res, timer }); this.queue.push(item);
